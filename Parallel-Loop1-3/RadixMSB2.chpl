 ...
 module RadixMSB2 {
   ...
   proc radixSortMSB(array:[?dom] int) {
     ...
     var auxArray:[1..array.size] atomic int;
     var place = findPlace(array);
     __radixSortMSB(array,place,dom.low,dom.high,auxArray);

   }
  
   private proc __radixSortMSB(array:[] int, place: int, startIndex: int, endIndex: int, auxArray:[] atomic int) {
     ...
     forall i in startIndex..endIndex {
       ...
       auxArray[auxIndex+1].write(array[i]);   
       curOffsets[countIndex].add(1, memory_order_relaxed);                
     }
   
     //swap slices of array and aux array   
     auxArray[1..(endIndex-startIndex+1)].exchange(array[startIndex..endIndex]);
